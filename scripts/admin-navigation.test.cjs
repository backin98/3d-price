...
const freshRun = context.test.uncertainHtml({desk:{shops:[]},catalog:{products:[],filaments:[]},jobs:[{id:'fresh',url:'https://market.samm.com/fdm-yazici',events:[{type:'extract',held:8,card:{url:'https://market.samm.com/bambu-lab-a1-combo-3d-yazici',name:'Bambu Lab A1 Combo'},decision:{action:'held'}}]}]});
assert.doesNotMatch(freshRun,/uncertain-hold/, 'the scraper held count is not a manual Removed state');
assert.match(freshRun,/Bambu Lab A1 Combo/);
const deferredRun={desk:{shops:[]},baseline:context.test.state.data.baseline,catalog:{products:[],filaments:[]},jobs:[{id:'deferred',url:'https://shop.example/3d',cards:{'https://shop.example/new':{url:'https://shop.example/new',name:'New printer',kind:'printer',price:123,reviewState:'uncertain',decision:{action:'create'}}}}]};
context.test.state.data=deferredRun;
assert.equal(context.test.collectUncertain(deferredRun).length,1,'a Shop Run card left without a baseline destination appears in Uncertain');
