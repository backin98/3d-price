...
let source=fs.readFileSync('netlify/functions/admin.mjs','utf8').replace(/^import .*;$/gm,'').replace('export default async','globalThis.handler = async');context.money=require('../lib/parse-money.cjs');context.matcher=require('../lib/product-match.cjs');vm.runInNewContext(source,context);
