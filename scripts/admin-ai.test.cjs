...
let source=fs.readFileSync('netlify/functions/admin.mjs','utf8').replace(/^import .*;$/gm,'').replace('export default async','globalThis.handler = async');context.money=require('../lib/parse-money.cjs');context.matcher=require('../lib/product-match.cjs');context.baselineLib=require('../lib/baseline-catalog.js');context.boardLib=require('../lib/baseline-board.cjs');vm.runInNewContext(source,context);
