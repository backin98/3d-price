...
async function layaPick(listing, candidates, opts = {}) {
  ...
  const response = await scorer(normalized.name, top.map(({ item }) => ({ id: item.id, name: item.name })), { signal: opts.signal });
  const scores = new Map((response.matches || []).map((m) => [m.id, Number(m.score) || 0]));
  const ordered = top.slice().sort((a, b) => (scores.get(b.item.id) || 0) - (scores.get(a.item.id) || 0));
  const hit = ordered[0];
  const confidence = scores.get(hit.item.id) || 0;
  const refusal = decidePair(normalized, hit.item);
  ...
  if (refusal.action === 'create' || refusal.conflict) return { action: 'hold', matchId: hit.item.id, confidence, reason: 'Laya cannot override Magellan hard conflict', rejected: 'conflict', candidates: shown };
