...
function pricesToTry(catalog, rates) {
  ...
  const fix = (p) => {
    ...
    const offers = (p.offers || []).filter((o) => Number.isFinite(Number(o.price)) && Number(o.price) > 0).map((o) => {
  ...
  return {
    ...(catalog || {}),
    products: ((catalog && catalog.products) || []).map(fix).filter((p) => p.offers.length || Number(p.price) > 0),
    filaments: ((catalog && catalog.filaments) || []).map(fix).filter((p) => p.offers.length || Number(p.price) > 0)
  };
}

// The stored catalog is the source of truth for prices. VAT is applied once, by the
// desk-driven re-price in the admin (see repriceShopVat), never invented per shop here.
module.exports = { unionCatalog, collapseByMagellan, pricesToTry };
