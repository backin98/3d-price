...
const NOT_THE_PRODUCT_PART = /\b(nozzle|nozul|hotend|hot end|extruder|bowden|yedek parca|yedek parça|plaka|plate|kabin|enclosure|tabla|termistor|thermistor|kayis|kayıs|filtre|sicaklik sensoru|için|icin)\b/;
function isNotTheProduct(name) {
  const t = ' ' + String(name || '').toLowerCase()
    .replace(/ı/g, 'i').replace(/İ/g, 'i')
    .replace(/[şğüöç]/g, (c) => ({ 'ş': 's', 'ğ': 'g', 'ü': 'u', 'ö': 'o', 'ç': 'c' }[c]))
    .replace(/[^a-z0-9]+/g, ' ') + ' ';
  return NOT_THE_PRODUCT_STRONG.test(t) || NOT_THE_PRODUCT_PART.test(t);
}
