...
const fold = s => String(s || '').toLocaleLowerCase('tr').replace(/ı/g, 'i').replace(/ş/g, 's').replace(/ç/g, 'c').replace(/ğ/g, 'g').replace(/ü/g, 'u').replace(/ö/g, 'o')
...
function urlSlug(url) {
  try {
    return decodeURIComponent(new URL(String(url || ''), 'https://catalog.invalid').pathname.split('/').filter(Boolean).pop() || '');
  } catch {
    return '';
  }
}

function modelCores(brandId, text) {
  let hay = ' ' + fold(text) + ' ';
  const found = [];
  const aliases = Object.entries(idx.MODELS[brandId] || {}).sort((a, b) => b[0].length - a[0].length);
  for (const [alias, core] of aliases) {
    const needle = ' ' + alias + ' ';
...
function laserWattsFromName(name) {
  ...
  // Fold Turkish so guc/güç and isitici/ısıtıcı compare the same way.
  for (const [a, b] of [["ı","i"],["İ","i"],["ş","s"],["ğ","g"],["ü","u"],["ö","o"],["ç","c"]]) t = t.split(a).join(b);
  const m = t.match(/([0-9]+)[ ]*(watt|w)/);
  if (!m) return "";
  // (c) Only a plausible laser wattage. Set from real data, not a guess: of 393 live catalog and
  // snapshot titles, the only wattages present are 10 (x8) and 40 (x4), all Bambu laser modules.
  // 1-80W covers those and other laser modules; 350W is a power supply.
  const watts = Number(m[1]);
  if (!(watts >= 1 && watts <= 80)) return "";
  // (b) Vetoed when a power or heater word sits within 2 tokens of the number. The window is tokens,
  // not characters, so a laser wattage elsewhere in a long title still counts.
  const power = ["adaptor", "guc", "kaynagi", "psu", "power", "supply", "isitici", "heater", "tabla", "bed"];
  const before = t.slice(0, m.index).trim().split(" ").filter(Boolean).slice(-2);
  const after = t.slice(m.index + m[0].length).trim().split(" ").filter(Boolean).slice(0, 2);
  if (power.some((word) => [...before, ...after].join(" ").includes(word))) return "";
  // (a) Otherwise the wattage IS the laser module - no laser word needed, which is what keeps
  // "H2C 10 Watt Combo" and "H2C Combo Laser 10 Watt" the same machine.
  return m[1];
}
...
function identity(p) {
  ...
  // A weak page title may contain only the brand. The product URL is useful only when its last
  // segment resolves through the same shared brand/model table; category and brand hubs do not.
  const title = (p.name || '') + ' ' + (p.variant || '');
  const slug = urlSlug(p.url);
  const titleAxes = axesFromTitle(title, { brandId: foldBrand(p.brand) });
  const brandId = titleAxes.brand || foldBrand(p.brand) || ((idx.brandOf(slug) || {}).id || '');
  const titleModel = idx.modelOf(brandId, title);
  const urlModel = !titleModel && idx.modelOf(brandId, slug);
  const axes = urlModel ? axesFromTitle(title + ' ' + slug, { brandId }) : titleAxes;
  const enrichedName = title + (urlModel ? ' ' + slug : '');
  ...
  return {
    ...
    ams,
    mini: /\bmini\b/.test(titleRules(enrichedName)) || /-mini$/.test(axes.modelCore || ''),
    laserW: laserWattsFromName(enrichedName) || (/(lazer|laser)/.test(titleRules(enrichedName)) ? 'laser' : ''),
    // Axes from the catalog index (see axesFromTitle).
    modelCore: axes.modelCore,
    modelCores: modelCores(brandId, title + (urlModel ? ' ' + slug : '')),
    titleIncomplete: p.kind !== 'filament' && p.kind !== 'resin' && !!brandId && !titleModel,
    urlModelCore: urlModel ? urlModel.id : '',
...
function conflicts(a, b) {
  ...
  //
  // A ONE-SIDED model is also a conflict. This check used to require both sides to be populated, so a
  // brand-only title ("Bambu Lab", "Bambu Lab Combo") never clashed with a real model, fell through to
  // the titleSubset merge, and welded thirteen other shops' offers onto one H2S card.
  // For printers a one-sided model is a conflict; for filament it cannot be, because a spool has no
  // model core in the first place and requiring both sides would split every colour variant pair.
  const realModelAxis = (x) => x.kind !== 'filament' && x.kind !== 'resin' && !!(x.brandId && idx.MODELS[x.brandId]);
  const modelStrict = a.kind === 'printer' || b.kind === 'printer' || realModelAxis(a) || realModelAxis(b);
  if ((modelStrict ? (a.modelCore || b.modelCore) : (a.modelCore && b.modelCore)) && a.modelCore !== b.modelCore) out.push('model');
  if (modelStrict && a.brandId && a.brandId === b.brandId) {
    const left = new Set(a.modelCores || []);
    const right = new Set(b.modelCores || []);
    if (([...left].some((x) => !right.has(x)) || [...right].some((x) => !left.has(x))) && !out.includes('model')) out.push('model');
  }
...
function titleSubset(a, b, ia, ib) {
  if ((ia && ia.titleIncomplete) || (ib && ib.titleIncomplete)) return false;
  const x = tokens(a);
  const y = tokens(b);
  ...
  if (extra.some((t) => VARIANT_WORDS.has(t))) return false;
  const left = new Set((ia && ia.modelCores) || []);
  const right = new Set((ib && ib.modelCores) || []);
  return [...left].every((x) => right.has(x)) && [...right].every((x) => left.has(x));
}
...
function decidePair(listing, candidate, visual) {
  ...
  if (printer && ia.titleIncomplete && ib.titleIncomplete && !ia.urlModelCore && !ib.urlModelCore) {
    const same = normalizePrinterTitle(listing.name) === normalizePrinterTitle(candidate.name);
    return { action: same ? 'review' : 'create', confidence: sim, reason: 'two incomplete printer identities', score: sim, candidateId: candidate.id, matchPath: 'magellan', conflict: !same };
  }
  if (printer && (ia.titleIncomplete || ib.titleIncomplete) && ia.modelCore && ia.modelCore === ib.modelCore) {
    return { action: 'merge', confidence: 0.96, reason: 'URL-derived model identity', score: Math.max(sim, 0.96), candidateId: candidate.id, matchPath: 'magellan' };
  }
  ...
  if (printer && titleSubset(listing.name, candidate.name, ia, ib)) {
