...
function identity(p) {
  ...
  const resolvedAms = axes.feederNamed && /^ams/.test(axes.feederGen) ? axes.feederGen : ams;
  ...
  return {
    ...
    feederGen: axes.feederGen || resolvedAms,
...
function decidePair(listing, candidate, visual) {
  ...
  if (printer && ia.titleIncomplete && ib.titleIncomplete && !ia.urlModelCore && !ib.urlModelCore) {
    ...
    return { action: same ? 'review' : 'create', confidence: sim, reason: 'two incomplete printer identities', score: sim, candidateId: candidate.id, matchPath: 'magellan' };
  }
