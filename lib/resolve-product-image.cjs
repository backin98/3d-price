...
const DECORATIVE = /(?:^|[\s_-])(badge|label|overlay|ribbon|sticker|watermark|corner)(?:$|[\s_-])/i;
...
function withoutDecorativeImages(html) {
  return String(html || '').replace(/<(div|span|figure)\b([^>]*)>[\s\S]*?<\/\1>/gi, (block, _tag, attrs) => {
    const classes = attr(attrs, 'class');
    return DECORATIVE.test(classes) ? block.replace(/<img\b[^>]*>/gi, '') : block;
  });
}

function resolveProductImage({ cardHtml, productPageHtml, pageUrl, productUrl } = {}) {
  const card = withoutDecorativeImages(cardHtml);
