...
//   2. A token matches the row's own words, or an offer's scraped title / shop — the row
...
//
// Browsers cannot require this file, so public/js/app.js mirrors it.

const { fold, index } = require('./product-match.cjs');
...
const tokenHits = (token, list) =>
  list.some((w, i) => w === token || w.startsWith(token) || near(w, token) || (list[i + 1] && w + list[i + 1] === token));

function searchableSourceTitle(product, title) {
  if (!title) return '';
  const value = fold(title);
  if (value === fold(product && product.brand)) return '';
  const brand = index.brandOf(title);
  return brand && (index.BRANDS[brand.id] || []).some((alias) => fold(alias) === value) ? '' : title;
}
...
function searchHaystack(product) {
  ...
  return [
    ...
    ...offers.flatMap((o) => [o.store, searchableSourceTitle(product, o.sourceTitle)])
  ].filter(Boolean).join(' ');
}
