...
async function checkOfferStock(url, { fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT, retries = RETRIES, renderHtml } = {}) {
  let last = 'unknown';
...
async function refreshStock({ catalog, limit = 40, staleHours = 12, now = Date.now(), budgetMs = DEFAULT_BUDGET, fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT, renderHtml, onProgress } = {}) {
  ...
  for (let i = 0; i < targets.length; i += CONCURRENCY) {
    ...
    const read = await Promise.all(batch.map((t) => checkOfferStock(t.offer.url, { fetchImpl, timeoutMs, renderHtml })));
    batch.forEach((t, k) => {
      const r = read[k];
      ...
      results.push({ url: t.offer.url, store: t.offer.store, before, after: r.status, verified: r.verified === true, checkedAt: t.offer.stockCheckedAt, method: r.method });
