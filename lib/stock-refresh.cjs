...
async function checkOfferStock(url, { fetchImpl = fetch, timeoutMs = DEFAULT_TIMEOUT } = {}) {
  ...
  try {
    ...
    const why = err && err.name === 'AbortError' ? 'timeout' : String((err && err.message) || err).slice(0, 40);
    return { status: 'unknown', quantity: null, verified: false, method: 'error:' + why };
  } finally {
    clearTimeout(timer);
  }
}
