...
function readStockPage(html) {
  ...
  const at = (() => {
    const m = buyRaw.match(/sepete\s*ekle|add\s*to\s*cart|sepetekle|addtocart|btn_sepet_ekle/i);
    return m ? m.index : -1;
  })();
  if (at >= 0) {
    const box = fold(stripTags(buyRaw.slice(Math.max(0, at - 400), at + 400)));
    if (PREORDER.test(box)) return { status: 'preorder', verified: true, method: 'buy-box-preorder' };
  }
  ...
  const price = buyRaw.search(/\d{1,3}(?:\.\d{3})+(?:,\d{2})?\s*(?:TL|₺)/i);
  ...
  if (anchor >= 0) {
    ...
    const box = fold(stripTags(buyRaw.slice(Math.max(0, anchor - 400), anchor + 400)));
...
function rollupProductStock(product, now) {
  ...
  product.stockVerified = offers.every((o) => o.stockVerified === true) && offers.length > 0;
