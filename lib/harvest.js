...
function readStockFromHtml(html, extras) {
  // Theme translations and script templates describe every stock state, not this listing.
  const raw = String(html || "").replace(/<(script|style|template|svg)\b[\s\S]*?<\/\1>/gi, " ");
  const active = raw
  ...
  if (oos && hasCart && extra.deferContradictoryCart) {
    return { status: "unknown", quantity: null, evidence: "product-page-required", verified: false };
  }
  if (oos && !fromStock) {
...
function discoverCards(html) {
  ...
  const strictCleanSlug = !group;
  const items = group ? group.items : [...String(html || "").matchAll(/<a\b([^>]*)>/gi)]
    .map((m) => ({ href: attr("a " + (m[1] || ""), "href"), pos: m.index }))
    .filter((item) => item.href && isLikelyProductUrl(item.href, ""));
  ...
  return strictCleanSlug && cards.length < DISCOVER_MIN_LINKS ? [] : cards;
}
...
function primaryHref(cardHtml, base, kind) {
  const schemaTag = (String(cardHtml || "").match(/<[^>]+\bitemprop=["']url["'][^>]*>/i) || [])[0] || "";
  const schemaHref = attr(schemaTag, "content") || attr(schemaTag, "href");
  const schemaUrl = schemaHref ? absUrl(schemaHref, base) : "";
  if (schemaUrl && isLikelyProductUrl(schemaUrl, kind || "printer")) {
    return { href: schemaUrl, score: -1, title: "" };
  }
...
function firstText(cardHtml, names) {
  ...
  const schemaTag = (String(cardHtml || "").match(/<[^>]+\bitemprop=["']name["'][^>]*>/i) || [])[0] || "";
  const schemaName = attr(schemaTag, "content");
  if (schemaName && !isBadgeText(schemaName)) return schemaName.trim();
...
function parseCard(block, base, kind, categoryUrl, opts) {
  ...
  const stock = readStockFromHtml(block.html, {
    badge,
    deferMissingCart: opts && opts.deferMissingCart,
    deferContradictoryCart: opts && opts.deferContradictoryCart
  }).status;
...
function vatIncludedPrice(html) {
  ...
  for (const hit of raw.matchAll(/(?:KDV\s*(?:Dahil|Dahildir)|vergi(?:ler)?\s*dahil)/gi)) {
    const before = raw.slice(Math.max(0, hit.index - 80), hit.index);
    const after = raw.slice(hit.index + hit[0].length, hit.index + hit[0].length + 80);
    const picked = moneyTokens(before).filter((token) => /^[\s:()[\]–-]*$/.test(before.slice(token.end))).at(-1)
      || moneyTokens(after).find((token) => /^[\s:()[\]–-]*$/.test(after.slice(0, token.index)));
    if (picked) return { price: picked.amount, currency: picked.currency || "", method: "vat-included-price" };
  }
  return null;
}

// Each configured storefront has one source that represents what its customer actually pays.
// Try that stable source first; the general evidence ranking below remains the fallback if a
// shop changes its markup and the preferred source disappears.
const PRICE_SOURCE_BY_HOST = {
  "robotzade.com": "structured",
  "3dcim.com": "cart-price",
  "3dteknomarket.com": "structured",
  "store.metatechtr.com": "vat-included-price",
  "robolinkmarket.com": "structured",
  "valment.com.tr": "cart-price",
  "robotizmo.net": "vat-included-price",
  "incehesap.com": "structured",
  "rhino3dprinter.com": "structured",
  "3dultra.com.tr": "cart-price",
  "urhanshop.com": "structured"
};

function preferredPriceSource(url) {
  try { return PRICE_SOURCE_BY_HOST[new URL(url).hostname.replace(/^www\./, "").toLowerCase()] || ""; }
  catch { return ""; }
}

function netPlusVatPrice(html) {
  const text = visibleText(html);
  const hit = text.match(/([0-9][0-9.,]*)\s*(?:TL|TRY|₺)?\s*\+\s*KDV/i);
  return hit ? parseMoney(hit[1]) : null;
}
...
function extractProductPage(html, url, kind) {
  ...
  const preferred = preferredPriceSource(url);
  const rawProfiled = preferred === "structured" ? structured
    : preferred === "cart-price" ? transaction
      : preferred === "vat-included-price" ? includedTotal : null;
  const profiled = rawProfiled && rawProfiled.price >= priceFloor("other", rawProfiled.currency || detectedCurrency)
    ? { price: rawProfiled.price, currency: rawProfiled.currency || detectedCurrency, raw: String(rawProfiled.price), source: rawProfiled.method }
    : null;
  if (profiled) {
    picked = profiled;
  } else if (includedTotal && believable.some((c) => c.source === "vat-included-price")) {
  ...
  let vatStatus = vatFromText(raw);
  const netPlusVat = netPlusVatPrice(raw);
  if (vatStatus === "excluded" && netPlusVat && Math.abs(picked.price - netPlusVat * 1.2) <= Math.max(1, picked.price * 0.002)) {
    vatStatus = "included";
  }
  const product = {
    url,
  ...
  if (!profiled && product.price < priceFloor(product.kind, product.currency || product.priceCurrency)) {
...
async function harvestCategory({ categoryUrl, kind, maxProducts = 400, html, urls, fetchImpl, inStockOnly = true } = {}) {
  ...
  const parseAll = (list) => {
    ...
    for (const card of list) {
      ...
      const parsed = parseCard(card, base, kind, categoryUrl, {
        inStockOnly,
        deferMissingCart,
        deferContradictoryCart: list.length > 1
      });
