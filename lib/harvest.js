...
const PRICE_SOURCE_BY_HOST = {
  ...
  "valment.com.tr": "vat-included-price",
...
function extractProductPage(html, url, kind) {
  ...
  const profiled = preferred === "structured"
    ? believable.find((c) => structured && c.source === structured.method)
    : believable.find((c) => c.source === preferred);
  if (profiled) {
    picked = profiled;
