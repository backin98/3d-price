...
const { parseMoney, pickPrice } = require("./parse-money.cjs");
...
function cardPrice(cardHtml) {
  ...
  for (const b of blocks) {
    const n = normalizeOfferPrice(stripTags(b.inner));
  ...
  const anchored = [...text.matchAll(/(?:\d{1,3}(?:\.\d{3})+|\d+)(?:,\d{2})?\s*(?:TL|₺|TRY)/gi)]
    .map((m) => ({ n: normalizeOfferPrice(m[0]), around: text.slice(Math.max(0, m.index - 32), m.index + m[0].length + 32) }))
    .filter((c) => c.n && !isJunkAmount(c.n, ctx) && !/kargo|cargo|teslimat|shipping|ucretsiz|ücretsiz|taksit/i.test(c.around));
  if (anchored.length) return anchored[0].n;
  const m = text.match(PRICE_RE);
  const n = m ? normalizeOfferPrice(m[0]) : null;
  if (n && !isJunkAmount(n, ctx)) return n;
  return null;
}
...
function parseCard(block, base, kind, categoryUrl, opts) {
  ...
  const price = cardPrice(block.html);
  ...
  const product = {
    ...
    kind: detectedType,
    price,
    currency: detectCurrency(block.html),
