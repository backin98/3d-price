...
function readStockFromHtml(html, extras) {
  ...
  if (hasPrice && !hasCart) return { status: "out_of_stock", quantity: null, evidence: "no-add-to-cart", verified: true };
...
function parseCard(block, base, kind, categoryUrl, opts) {
  ...
  const stock = readStockFromHtml(block.html, { badge }).status;
...
function extractProductPage(html, url, kind) {
  ...
  if (!picked) {
    ...
    return {
      incomplete: { url, name, brand, kind: classifyProductType(name, brand), price: null, priceRejected: rejected, priceCurrency: detectedCurrency || "TRY", image, vatIncluded: vatFromText(raw) !== "excluded" },
      reason: rejected
