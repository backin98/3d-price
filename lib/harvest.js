...
function firstText(cardHtml, names) {
  ...
  const blocks = extractElements(cardHtml, (tag, attrs) => {
    ...
    return classList(attrs).some((c) => want.has(c));
  });
...
function cardPrice(cardHtml) {
  ...
  const m = stripTags(withoutOld).match(PRICE_RE);
...
async function harvestCategory({ categoryUrl, kind, maxProducts = 400, html, urls, fetchImpl, inStockOnly = true } = {}) {
  ...
  const warning = !cards.length ? "no product grid on this URL (use a listing/collection page, not the homepage)" : "";
  return { inScope, mismatches, rejected, warning };
}
