...
async function runWebsiteJob(job, emit) {
  ...
  for (let offset = 0; offset < needPage.length; offset += 6) {
    await Promise.all(needPage.slice(offset, offset + 6).map(async (pageUrl, batchIndex) => {
...
module.exports = { runWebsiteJob, paginationMisses, listingFromHarvest };
