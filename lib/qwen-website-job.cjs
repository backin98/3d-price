...
const { fetchHtml, listingPageUrls } = require('./ai-scraper.cjs');
...
const paginationMisses = (url, added, misses = 0) => /[?&](tp|sayfa|pg|page|ps)=\d+/i.test(url) ? (added ? 0 : misses + 1) : misses;
...
function listingFromHarvest(url, card, job, stockFilterUrls) {
  ...
  const vatExcluded = job.vat === 'excluded' || card.vatExcluded === true;
  ...
  return {
    ...
    vatForced: card.vatExcluded === true
  };
}
...
function isNavUrl(href, kind, currentPath) {
  ...
  const paginated = /[?&](tp|sayfa|pg|page)=\d+/i.test(href);
...
async function runWebsiteJob(job, emit) {
  ...
  const fetchPage = async url => {
    ...
    const listing = /kategori|collections|yazicilar|filament|\/3d-yazici/i.test(u);
    ...
    let imgTries = 0;
    for (const p of harvested.inScope) {
      if (p.image || imgTries >= 20) continue;
      imgTries += 1;
      try {
    ...
    const pageLinks = unique.filter(x => /[?&](sayfa|tp|pg|page)=\d+/i.test(x.url) && !String(x.url).includes('{{')).map(x => x.url);
    return { url: u, html, image: '', text: clean(html).slice(0, 22000), links: unique.slice(0, 250), productUrls, pageLinks, thumbs, harvested };
  };

  log({ type: 'log', stage: 'boot', text: 'Starting at ' + start.href });
  ...
  while (queue.length && seen.size < maxPages && products.size < maxProducts) {
    ...
    if (emptyPaginationPages >= 2) {
      for (let i = queue.length - 1; i >= 0; i -= 1) if (/[?&](tp|sayfa|pg|page|ps)=\d+/i.test(queue[i])) queue.splice(i, 1);
      log({ type: 'log', stage: 'discover', text: 'Pagination stopped after two pages added no buyable products.' });
    } else {
      const extra = listingPageUrls(page.url, page.html).find((x) => !seen.has(x) && !queue.includes(x));
    ...
    for (const raw of emptyPaginationPages >= 2 ? [] : page.pageLinks) {
      try {
  ...
  for (let i = 0; i < needPage.length; i++) {
    if (stopped()) throw Error('Stopped');
    try {
      const page = await fetchPage(needPage[i]);
        ...
        if (partial && partial.name && Number.isFinite(partial.price) && partial.price > 0 && partial.image) {
          ...
          if (brand) {
            listings.push({
              ...
              stockStatus: readStockFromHtml(page.html, { filterVerified, harvested: harvested.stock, badge: harvested.stockBadge }).status,
      ...
      const stock = readStockFromHtml(page.html, {
        filterVerified,
        harvested: harvested.stock || x.stock,
      ...
      const forcedVat = x.plusVat === true;
      const vatExcludedHere = forcedVat || (job.vat === 'excluded' && !x.vatIncluded);
      ...
      job.signal?.throwIfAborted();
      errors.push({ url: needPage[i], error: e.message });
      log({ type: 'extract', done: listings.length + i + 1, total: urls.length, accepted: listings.length, held: errors.length, url: needPage[i], error: e.message });
    }
  }
  ...
  let placed;
  try {
    placed = await placeListings({ listings, site, catalogFile: catalog, apply: job.applyToCatalog === true, emit: log, runDir: run, signal: job.signal, autoLlmMatch: job.autoLlmMatch === true, visualMatch: job.visualMatch !== false });
  } finally {
...
module.exports = { runWebsiteJob, paginationMisses };
