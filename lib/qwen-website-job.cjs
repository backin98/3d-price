...
function listingFromHarvest(url, card, job, stockFilterUrls) {
  ...
  if ((stock.status || card.stock) === 'out_of_stock') return null;
...
async function runWebsiteJob(job, emit) {
  ...
  const fetchPage = async url => {
    ...
    for (const p of harvested.inScope) {
      if (p.image || pdpTries >= 40) continue;
      pdpTries += 1;
      try {
        const pdp = await fetchHtml(p.url, job.signal, { scroll: false, discoverStock: false });
  ...
  while (queue.length && seen.size < maxPages && products.size < maxProducts) {
    ...
    for (const extra of listingPageUrls(page.url, page.html)) {
    ...
    for (const raw of page.pageLinks) {
      try {
        const u = sameOrigin(raw);
        if (!seen.has(u) && !queue.includes(u) && inScope(u) && isNavUrl(u, job.kind, new URL(page.url).pathname)) queue.push(u);
      } catch (e) {
