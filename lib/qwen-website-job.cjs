...
async function runWebsiteJob(job, emit) {
  ...
  for (let i = 0; i < needPage.length; i++) {
    ...
    try {
      ...
      let price = x.price;
      if (x.plusVat || (job.vat === 'excluded' && !x.vatIncluded)) price = withVat(price, 'excluded');
      const listing = {
  ...
  const placed = await placeListings({ listings, site, catalogFile: catalog, apply: job.applyToCatalog === true, emit: log, runDir: run, signal: job.signal, autoLlmMatch: job.autoLlmMatch === true });
  job.signal?.throwIfAborted();
