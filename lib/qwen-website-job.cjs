...
const shouldScrollPage = (url, startUrl) => new URL(url).pathname === new URL(startUrl).pathname
  || /kategori|collections|collection|yazicilar|filament|\/3d-yazici|\/3d-printer|products|katalog/i.test(url);
...
async function runWebsiteJob(job, emit) {
  ...
  const fetchPage = async url => {
    ...
    const listing = shouldScrollPage(u, start.href);
...
module.exports = { runWebsiteJob, paginationMisses, oosPagingStop, listingFromHarvest, shouldAddVat, shouldScrollPage };
